...
class Wiki_Premium {
	...
	public function admin_page_settings() { ?>
		<tr valign="top">
			<th><label
					for="incsub_wiki-breadcrumbs_in_title"><?php _e( 'Number of breadcrumbs to add to title', 'wiki' ); ?></label>
			</th>
			<td><input type="text" size="2" id="incsub_wiki-breadcrumbs_in_title" name="wiki[breadcrumbs_in_title]"
			           value="<?php echo $this->wiki->get_setting( 'breadcrumbs_in_title' ); ?>"/></td>
		</tr>
		<tr valign="top">
			<th><label for="incsub_wiki-wiki_name"><?php _e( 'What do you want to call Wikis?', 'wiki' ); ?></label>
			</th>
			<td><input type="text" size="20" id="incsub_wiki-wiki_name" name="wiki[wiki_name]"
			           value="<?php echo $this->wiki->get_setting( 'wiki_name' ); ?>"/></td>
		</tr>
		<tr valign="top">
			<th><label
					for="incsub_wiki-sub_wiki_name"><?php _e( 'What do you want to call Sub Wikis?', 'wiki' ); ?></label>
			</th>
			<td><input type="text" size="20" id="incsub_wiki-sub_wiki_name" name="wiki[sub_wiki_name]"
			           value="<?php echo $this->wiki->get_setting( 'sub_wiki_name' ); ?>"/></td>
		</tr>
		<tr valign="top">
			<th><label
					for="incsub_wiki-sub_wiki_order_by"><?php _e( 'How should Sub Wikis be ordered?', 'wiki' ); ?></label>
			</th>
			<td>
				<select id="incsub_wiki-sub_wiki_order_by" name="wiki[sub_wiki_order_by]">
					<option
						value="menu_order" <?php selected( $this->wiki->get_setting( 'sub_wiki_order_by' ), 'menu_order' ); ?>>
					...
					<option
						value="title" <?php selected( $this->wiki->get_setting( 'sub_wiki_order_by' ), 'title' ); ?>>
						<?php _e( 'Title', 'wiki' ); ?>
					</option>
					<option value="rand" <?php selected( $this->wiki->get_setting( 'sub_wiki_order_by' ), 'rand' ); ?>>
						<?php _e( 'Random', 'wiki' ); ?>
					</option>
				</select>
			</td>
		...
		<tr valign="top">
			<th><label
					for="incsub_wiki-sub_wiki_order"><?php _e( 'What order should Sub Wikis be ordered?', 'wiki' ); ?></label>
			</th>
			<td>
				<select id="incsub_wiki-sub_wiki_order" name="wiki[sub_wiki_order]">
					<option
						value="ASC" <?php selected( $this->wiki->get_setting( 'sub_wiki_order' ), 'ASC' ); ?>><?php _e( 'Ascending', 'wiki' ); ?></option>
					<option
						value="DESC" <?php selected( $this->wiki->get_setting( 'sub_wiki_order' ), 'DESC' ); ?>><?php _e( 'Descending', 'wiki' ); ?></option>
				</select>
			</td>
		</tr>
		<tr valign="top">
			...
			<td>
				...
				foreach ( $editable_roles as $role_key => $role ) {
					...
					?>
					<label><input type="checkbox" name="edit_wiki_privileges[<?php echo $role_key; ?>]"
					              value="<?php echo $role_key; ?>" <?php echo $role_obj->has_cap( 'edit_wiki_privileges' ) ? 'checked="checked"' : ''; ?> /> <?php echo $role['name']; ?>
					</label><br/>
					<?php
				}
				?>
	...
	public function add_meta_boxes( $post ) {
		if ( $post->post_author == wp_get_current_user()->ID || current_user_can( 'edit_posts' ) ) {
			add_meta_box( 'incsub-wiki-privileges', __( 'Wiki Privileges', 'wiki' ), array(
				&$this,
				'privileges_meta_box'
			), 'incsub_wiki', 'side' );
		}

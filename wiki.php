...
class Wiki {
	...
	private function __construct() {
		...
		add_filter( 'the_content', array(
			&$this,
			'theme'
		), 999 );    //set to really low priority. we want this to run after all other filters, otherwise undesired output may result.
	...
	function non_suppporter_admin_menu() {
		global $psts;
		add_menu_page( __( 'Wiki', 'wiki' ), __( 'Wiki', 'wiki' ), 'edit_posts', 'incsub_wiki', array(
			&$psts,
			'feature_notice'
		), null, 30 );
	}
			...
			default:
				...
				if ( $notification_meta == 'enabled' && ! $this->is_subscribed() ) {
					if ( is_user_logged_in() ) {
						$bottom .= '<div class="incsub_wiki-subscribe"><a href="' . wp_nonce_url( add_query_arg( array(
								'post_id'   => $post->ID,
								'subscribe' => 1
							) ), "wiki-subscribe-wiki_$post->ID" ) . '">' . __( 'Notify me of changes', 'wiki' ) . '</a></div>';
					} else {
	...
	function tabs() {
		...
		if ( $post && current_user_can( $post_type_object->cap->edit_post, $post->ID ) ) {
			...
			$tabs .= '<li class="' . join( ' ', $classes['create'] ) . '"><a href="' . add_query_arg( array(
					'action'  => 'edit',
					'eaction' => 'create'
				), $permalink ) . '">' . __( 'Create new', 'wiki' ) . '</a></li>';
			$tabs .= '</ul>';
		}
		...
		foreach ( $revisions as $revision ) {
			...
			if ( 'form-table' == $format ) {
				if ( $left ) {
					...
					$left_checked = ( isset( $right_checked ) && $right_checked ) ? ' checked="checked"' : '';
				} // [sic] (the next one)
				...
				if ( $post->ID != $revision->ID && $can_edit_post && current_user_can( 'read_post', $revision->ID ) ) {
					$actions = '<a href="' . wp_nonce_url( add_query_arg( array(
							'revision' => $revision->ID,
							'action'   => 'restore'
						) ), "restore-post_$post->ID|$revision->ID" ) . '">' . __( 'Restore' ) . '</a>';
				} else {
					$actions = ' ';
				}
	...
	function register_post_types() {
		...
		register_post_type( 'incsub_wiki', array(
				...
				'capability_type' => 'wiki',
					...
					'edit_others_posts' => 'edit_other_wikis',
	...
	function meta_boxes() {
		...
		if ( $post->post_author == $current_user->ID || current_user_can( 'edit_posts' ) ) {
			add_meta_box( 'incsub-wiki-notifications', __( 'Wiki E-mail Notifications', 'wiki' ), array(
				&$this,
	...
	function name_save( $post_name ) {
		if ( !empty( $_POST['post_type'] ) && $_POST['post_type'] == 'incsub_wiki' && empty( $post_name ) ) {
